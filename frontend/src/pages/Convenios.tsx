...
export default function Convenios() {
  ...
  const [tiposAlmuerzoOptions, setTiposAlmuerzoOptions] = useState<{ code: string, label: string }[]>([]);
  ...
  const fetchTiposAlmuerzo = async () => {
    try {
  ...
  return (
    <div className="space-y-6">
          ...
          <Tabs defaultValue="info" className="w-full mt-4">
            ...
            <TabsContent value="info" className="space-y-4 py-4">
              ...
              <div className="space-y-3 pt-2 border-t mt-4">
                <Label className="text-sm font-bold text-primary">Tipos de Almuerzo Permitidos</Label>
                <p className="text-xs text-muted-foreground">Seleccione los tipos de almuerzo que los colaboradores de esta empresa podrán solicitar mediante el bot de Telegram.</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {tiposAlmuerzoOptions.map((tipo) => (
                    <div key={tipo.code} className="flex flex-row items-center space-x-3 space-y-0 rounded-md border p-3 hover:bg-muted/50 transition-colors">
                      <Checkbox
                        id={`tipo-${tipo.code}`}
                        checked={(formData.tipos_almuerzo_permitidos || []).includes(tipo.code)}
                        onCheckedChange={(checked) => {
                          if (checked) {
                            setFormData({ ...formData, tipos_almuerzo_permitidos: [...formData.tipos_almuerzo_permitidos, tipo.code] });
                          } else {
                            setFormData({ ...formData, tipos_almuerzo_permitidos: formData.tipos_almuerzo_permitidos.filter((t: string) => t !== tipo.code) });
                          }
                        }}
                      />
                      <div className="space-y-1 leading-none">
                        <Label htmlFor={`tipo-${tipo.code}`} className="text-sm font-medium leading-none cursor-pointer">
                          {tipo.label}
                        </Label>
                      </div>
                    </div>
              ...
              <DialogFooter className="pt-4 sm:justify-between w-full flex flex-col sm:flex-row gap-2">
                {editingConvenio ? (
                  <Button
                    variant="outline"
                    onClick={(e) => {
                ...
                <div className="flex gap-2 justify-end">
                  ...
                  <Button onClick={handleSave} disabled={isSaving} className="bg-cafe hover:bg-cafe/90 shadow-lg shadow-cafe/20">{isSaving ? 'Guardando...' : 'Guardar Datos'}</Button>
                </div>
              </DialogFooter>
            </TabsContent>
      ...
      <ConvenioToggleStatusDialog
        open={isAlertOpen}
        onOpenChange={setIsAlertOpen}
        convenioToToggle={convenioToToggle}
        onConfirm={confirmToggle}
      />

      <ConvenioRenewalDialog
        open={isRenewalDialogOpen}
        onOpenChange={setIsRenewalDialogOpen}
        convenioToRenew={convenioToRenew}
        renewalDates={renewalDates}
        setRenewalDates={setRenewalDates}
        isSaving={isSaving}
        onRenew={handleRenew}
      />

      <ConvenioReportDialog
        open={isReportDialogOpen}
        onOpenChange={setIsReportDialogOpen}
        reportConvenio={reportConvenio}
