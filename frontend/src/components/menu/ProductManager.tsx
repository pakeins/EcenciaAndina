...
export function ProductManager({ onProductsChanged }: { onProductsChanged: () => void }) {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('¿Seguro que deseas eliminar este producto? No se podrá eliminar si está en menús anteriores.')) return;
    try {
      const res = await apiFetch(`/alimentos/${id}`, { method: 'DELETE' });
      if (res.ok) {
            ...
            {loading ? (
              ...
              filteredProducts.map(p => (
                <div key={p.id} className="flex justify-between items-center p-3 hover:bg-muted/50">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => handleDelete(p.id)} className="text-destructive hover:text-destructive hover:bg-destructive/10 h-8 w-8 p-0">
